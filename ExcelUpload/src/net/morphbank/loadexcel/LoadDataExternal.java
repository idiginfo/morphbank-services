...
//UserId and Password arguments added for usage by other morphbank sites.
public class LoadDataExternal {
...
//		String dbHost = "dev.morphbank.net";
		...
		if (args.length == 0) {
			System.out.println("usage: java net.morphbank.loadexcel.LoadData.java filename"
					+ " [dbhost] [dbname] [userid] [password] [imagepath] [debug]");
			// System.exit(-1);
		}
		if (args.length > 0) filename = args[0];
		if (args.length > 1) dbHost = args[1];
		if (args.length > 2) dbName = args[2];
		if (args.length > 3) dbUserId = args[3];
		if (args.length > 4) dbPassword = args[4];
		if (args.length > 5) imageDirectoryPath = args[5];
		if (args.length > 6) debug = "debug".equals(args[6]);

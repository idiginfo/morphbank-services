...
public class MapSheetSpecimen {
	...
	public void setXmlSpecimenFields(XmlBaseObject xmlSpecimen) {
		...
		xmlSpecimen.addDescription(specimen.getValue(fields
				.getS_SPECIMEN_DESCRIPTION()));
		// xmlSpecimen.addUserProperty("SpecimenDescription",
		// specimen.getValue("Specimen Description"));
		xmlSpecimen.addUserProperty("SpecimenDescription",
				specimen.getValue(fields.getS_SPECIMEN_DESCRIPTION()));
		...
		xmlSpecimen.addDarwinTag(objectFactory.createDateIdentified(specimen
				.getValueDate(fields.getS_DATE_DETERMINED())));
		...
		xmlSpecimen.addDarwinTag(objectFactory
				.createOtherCatalogNumbers(specimen.getValue(fields
						.getS_PREVIOUS_CATALOG_NUMBER())));
		xmlSpecimen.addDarwinTag(objectFactory
				.createRelatedCatalogedItems(specimen.getValue(fields
						.getS_RELATED_CATALOG_ITEM())));
		...
		xmlSpecimen.addDarwinTag(objectFactory.createSex(specimen
				.getValue(fields.getS_SEX())));
		xmlSpecimen.addDarwinTag(objectFactory.createTypeStatus(specimen
				.getValue(fields.getS_TYPE_STATUS())));
		...
		xmlSpecimen.addDarwinTag(objectFactory.createCollector(specimen
				.getValue(fields.getS_COLLECTOR_NAME())));
		xmlSpecimen.addDarwinTag(objectFactory.createFieldNotes(specimen
				.getValue(fields.getS_NOTES())));

		xmlSpecimen.addDarwinTag(objectFactory
				.createEarliestDateCollected(specimen.getValueDate(fields
						.getS_EARLIEST_DATE_COLLECTED())));
		xmlSpecimen.addDarwinTag(objectFactory
				.createEarliestDateCollected(specimen.getValueDate(fields
						.getS_LATEST_DATE_COLLECTED())));

		// Locality properties
		xmlSpecimen.addDarwinTag(objectFactory.createContinent(specimen
				.getValue(fields.getS_CONTINENT())));
		xmlSpecimen.addDarwinTag(objectFactory.createWaterBody(specimen
				.getValue(fields.getS_OCEAN())));
		xmlSpecimen.addDarwinTag(objectFactory.createWaterBody(specimen
				.getValue(fields.getS_WATER_BODY())));
		xmlSpecimen.addDarwinTag(objectFactory.createCountry(specimen
				.getValue(fields.getS_COUNTRY())));
		xmlSpecimen.addDarwinTag(objectFactory.createStateProvince(specimen
				.getValue(fields.getS_STATE_OR_PROVINCE())));
		xmlSpecimen.addDarwinTag(objectFactory.createCounty(specimen
				.getValue(fields.getS_COUNTY())));
		xmlSpecimen.addDarwinTag(objectFactory.createLocality(specimen
				.getValue(fields.getS_LOCALITY())));
		xmlSpecimen.addDarwinTag(objectFactory
				.createInformationWithheld(specimen.getValue(fields
						.getS_INFORMATION_WITHHELD())));
		xmlSpecimen.addDarwinTag(objectFactory
				.createDecimalLatitude(getDouble(specimen.getValue(fields
						.getS_LATITUDE()))));
		xmlSpecimen.addDarwinTag(objectFactory
				.createDecimalLongitude(getDouble(specimen.getValue(fields
						.getS_LONGITUDE()))));
		// xmlSpecimen.addDarwinTag(objectFactory.createCoordinatePrecision(specimen
		// .getValue("Coordinate Precision")));
		xmlSpecimen.addDarwinTag(objectFactory
				.createMinimumElevationInMeters(getDouble(specimen
						.getValue(fields.getS_MINIMUM_ELEVATION()))));
		xmlSpecimen.addDarwinTag(objectFactory
				.createMaximumElevationInMeters(getDouble(specimen
						.getValue(fields.getS_MAXIMUM_ELEVATION()))));

		xmlSpecimen.addDarwinTag(objectFactory
				.createMinimumDepthInMeters(getDouble(specimen.getValue(fields
						.getS_MINIMUM_DEPTH()))));
		xmlSpecimen.addDarwinTag(objectFactory
				.createMaximumDepthInMeters(getDouble(specimen.getValue(fields
						.getS_MAXIMUM_DEPTH()))));
		xmlSpecimen.addUserProperty("Institution",
				specimen.getValue(fields.getS_INSTITUTION_NAME()));
		xmlSpecimen.addUserProperty("LocalityDescription",
				specimen.getValue(fields.getS_LOCALITY_DESCRIPTION()));
		Extref extref = MapSpreadsheetToXml.getExternalLink(specimen,
				"Specimen");
		if (extref != null)
			xmlSpecimen.addExternalRef(extref);
			...
			if (property[1].equalsIgnoreCase("specimen")) {
				if (property[3] == null || property[3].equalsIgnoreCase("")) { // namespace
					...
					if (property[2] == null || property[2].equalsIgnoreCase("")) { // property
																					// name
																					// empty
						xmlSpecimen.addUserProperty(property[0],
								specimen.getValue(property[0]));
					} else { // property name present
						xmlSpecimen.addUserProperty(property[2],
								specimen.getValue(property[0]));
					}
					...
					if (property[2] == null || property[2].equalsIgnoreCase("")) { // property
																					...
																					// empty
						xmlSpecimen.addUserProperty(property[0],
								specimen.getValue(property[0]), property[3]);
					} else { // property name present
						xmlSpecimen.addUserProperty(property[2],
								specimen.getValue(property[0]), property[3]);
					}
	...
	protected String getDegMinSec(String degrees, String minutes,
			String seconds, String direction) {
		if (degrees == null || degrees.length() < 1)
			return null;
		String result = degrees + DEGREE + minutes + "' " + seconds + "\" "
				+ direction;
		return result;
	}
		...
		if (this.specimen instanceof XlsFieldMapper) {
			...
			if (cols != 4) {
				MorphbankConfig.SYSTEM_LOGGER
						.info("Wrong number of colums in Excel spreadsheet.");
				return null;
			}

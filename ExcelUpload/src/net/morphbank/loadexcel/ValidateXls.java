...
public class ValidateXls {
	...
	public static void main(String[] args) {
		SheetReader sheetReader = new SheetReader("/home/gjimenez/Downloads/MatthewBuffington/mb3a_Oberthuerellinae03.xls", null);
	...
	private boolean checkCredentials() {
		...
		for (int i= 3; i <= 7; i++) {
			if (credentials.getCell(1, i) == null || credentials.getCell(1, i).getContents().equalsIgnoreCase("")) {
				System.out.println(credentials.getCell(0, i).getContents().replaceAll(":", "") + " cannot be empty.<br />");
				output.append(credentials.getCell(0, i).getContents().replaceAll(":", "") + " cannot be empty.<br />");
				anyEmpty = true;
			}
	...
	private boolean checkDateFormat() {
		...
		for (int i = 1; i < specimenDescription.length; i++) {
			...
			if (date.equalsIgnoreCase("#VALUE!") || !this.dateOnSpecimenDescription(date, datePresent)) {
				System.out.println("Date Collected row " + (i + 1)  + " does not have the format yyyy-mm-dd");
				output.append("Date Collected row " + (i +1) + " does not have the format yyyy-mm-dd.<br />");
				correctFormat = false;
			}

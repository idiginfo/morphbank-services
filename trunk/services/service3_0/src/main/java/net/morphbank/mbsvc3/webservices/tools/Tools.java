...
public class Tools {
	...
	private static final String[] FILE_EXTENSIONS = { ".tif", ".tiff", ".jpg",
			".jpeg", ".bmp", ".gif", ".png" };
	...
	public static String createFolder(String fileName) {
		...
		boolean success = new File(MorphbankConfig.getFilepath() + folder)
				.mkdir();
		if (!success) {
	...
	public static void eraseTempFile(String folderPath, String fileName,
			boolean eraseFolder) {
	...
	public static boolean fileExtensionOk(String fileName) {
		...
		for (int i = 0; i < FILE_EXTENSIONS.length; i++) {
			if (extension.equalsIgnoreCase(FILE_EXTENSIONS[i]))
				return true;
		}
		return false;
	}
	...
	/**
	 * Inforce yyyy-mm-dd format to a cell Split the date and check each part
	 * individually
	 * 
	 * @param cell
	...
	public static boolean isEmpty(Cell cell) {
		...
		if (cell.getCellType() == Cell.CELL_TYPE_STRING) {
			...
			if (DateUtil.isCellDateFormatted(cell)) {// returns null for blank
														// cells

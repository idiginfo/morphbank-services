...
public class Image extends BaseObject implements Serializable {
	...
	public Image(int id, String name, String ownerId, String submitterId, String groupId) {
	...
	public Image(
			// required parameters
			int id, User user, User submittedBy, Group group, Specimen specimen,
			View view, String imageType, int accessNum, String copyrightText,
			int resolution) {
	...
	/**
	 * Set accessNum field before initializing the BaseObject field and
	 * committing to the database
	 */
	public boolean persist() {
		boolean superPersist = super.persist();
		if (getAccessNum() == 0) {
			// set access number field
			setAccessNum(getNewAccessNum());
			setThumbURL(Integer.toString(getId()));// image is its own thumbnail
			MorphbankConfig.getEntityManager().flush();
		}
		return superPersist;
	}

	protected int getNewAccessNum() {
		Query sessionQuery = MorphbankConfig.getEntityManager()
				.createNativeQuery("select max(i.accessNum) from Image i");
	...
	public boolean validate() {
		...
		if (imageType == null || imageUser == null || imageGroup == null) {
			return false;
		} else if (copyrightText == null || (resolution!=null && resolution < 0)
				|| imageDateToPublish == null || creativeCommons == null) {
			return false;
		}

...
public class ExtLinkTest {
	...
	public static final String[] EXT_IDS = { "Chicago Bo51365", "XXXX",
			"PlantColle51372", "Dentary im51431" };
	public static final String KEY = "testfield2";
	public static final String VALUE = "testvalue1";
	public static final String EXT_UNIQUE = "External Unique Reference";
	...
	public static void main(String[] args) {
		...
		ExtLinkTest test = new ExtLinkTest();
		test.run();
	}

	public void run() {
		//runLocalityTest(80004); // locality
		int id = ExternalLinkType.getExternalTypeId("Determination Resource");
		System.out.println ("Determination Resource: "+id);
		id = ExternalLinkType.getExternalTypeId("Other");
		System.out.println ("Other: "+id);
		id = ExternalLinkType.getExternalTypeId(EXT_UNIQUE);
		System.out.println ("EXT_UNIQUE: "+id);
	...
	public void addImage() {
		...
		Image image = new Image(0, fred, fred, fred.getGroup(), specimen, view,
				"JPG", "copyright@2007 Fred", 100);
	...
	public List getUsers(String lastName) {
		List users = MorphbankConfig.getEntityManager().createQuery(
				"from User where id=77420").getResultList();
		...
		if (obj != null) {
			...
			if (obj instanceof Specimen) {
				...
				System.out.println("StateProvince: "
						+ locality.getStateProvince());

			}
			System.out.println("KEY Field " + field.getName() + " has value "
					+ field.getValue());
			Iterator<Map.Entry<String, UserProperty>> fields = obj
					.getUserProperties().entrySet().iterator();
	...
	public void runExtIdObjTest() {
		for (int i = 0; i < EXT_IDS.length; i++) {
			ExternalLinkObject obj = ExternalLinkObject
					.getExternalLinkObject(EXT_IDS[i]);
			System.out.print("The object with external id '" + EXT_IDS[i]);
			if (obj == null) {
				System.out.println("' not found");
			} else {
	...
	public void runExtIdBaseoObjectTest() {
		for (int i = 0; i < EXT_IDS.length; i++) {
			...
			if (obj == null) {
				...
				System.out.println(" with linkId "
						+ obj.getExternalLinks().get(0).getLinkId());
			}
		}
	}
}
